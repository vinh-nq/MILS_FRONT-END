...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertWater(
        value,
        `Insert New Water Rain [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  const handleEditItem = async (value) => {
    await dataDictionaryApi
      .UpdateWater(
        value,
        `Update Water Rain [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      title={`${typeModal === "add" ? t("add") : t("edit")} ${t("WATER")}`}
      visible={visible}
      width="630px"
      okButtonProps={{
        form: "form-water-management",
        key: "submit",
        htmlType: "submit",
        type: "primary",
      }}
      ...
      forceRender
    >
      <Form id="form-water-management" form={form} onFinish={handleSubmit}>
        <div>
          <span>{t("waterId")}</span>
