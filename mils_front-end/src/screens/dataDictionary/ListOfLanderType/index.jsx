...
let timeOut = "";
export default function ListOfLanderType(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_LANDER_TYPE);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.LIST_OF_LANDER_TYPE) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  const columns = [
    {
      title: t("landerTypeId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("landerTypeLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_LANDER_TYPE}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_LANDER_TYPE,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFLANDERTYPE")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_LANDER_TYPE}?keyword=${keyword}`);
        }}
      />
    </div>
  );
