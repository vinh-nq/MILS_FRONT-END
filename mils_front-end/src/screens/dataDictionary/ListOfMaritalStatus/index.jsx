...
let timeOut = "";
export default function ListOfMaritalStatus(props) {
  const { t } = useTranslation();
  const history = useHistory();
  const [keyword, setKeyword] = useState(null);
  const [page, setPage] = useState("1");
  const [checkLoading, setCheckLoading] = useState(false);
  const [listMatitalStatus, setListMatitalStatus] = useState([]);
  //Modal Items
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_MARITAL_STATUS);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (
        `${location.pathname}${location.search}` === PATH.LIST_OF_MARITAL_STATUS
      ) {
        setKeyword("");
      }
    });
  ...
  const fetchDataProvince = async (location) => {
    ...
    await dataDictionaryApi
      ...
      .then((res) => {
        ...
        setListMatitalStatus(res.data.Data);
      })
      .catch((error) => {
        setCheckLoading(false);
        messageError({
  ...
  const columns = [
    {
      title: t("maritalStatusId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("maritalStatusLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_MARITAL_STATUS}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_MARITAL_STATUS,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFMARITALSTATUS")}</span>
      ...
      <Table
        ...
        pagination={{
          current: Number(page),
          pageSize: 12,
          total: listMatitalStatus.length,
          onChange: (page) => {
            onChangePage(page);
          },
      ...
      <ModaItem
        ...
        typeModal={typeModal}
        arrayDuplicate={listMatitalStatus.map((el) => `${el.Id}`)}
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_MARITAL_STATUS}?keyword=${keyword}`);
        }}
      />
    </div>
  );
