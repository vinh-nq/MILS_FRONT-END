...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertMaritalStatus(
        value,
        `Insert New Marital Status [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  const handleEditItem = async (value) => {
    await dataDictionaryApi
      .UpdateMaritalStatus(
        value,
        `Update Marital Status [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      ...
      width="630px"
      okButtonProps={{
        form: "form-maritalStatus-management",
        key: "submit",
        htmlType: "submit",
        type: "primary",
      }}
    ...
    >
      <Form
        id="form-maritalStatus-management"
        form={form}
        onFinish={handleSubmit}
      >
        <div>
          <span>{t("maritalStatusId")}</span>
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  ...
                  value,
                  {
                    ...objectValidateForm.maritalStatusId,
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.maritalStatusLao,
                  t
                );
              },
            },
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.maritalStatusEng,
                  t
                );
              },
            },
