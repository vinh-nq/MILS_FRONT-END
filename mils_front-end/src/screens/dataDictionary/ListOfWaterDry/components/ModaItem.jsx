...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertWaterDry(
        value,
        `Insert New Water Dry [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      title={`${typeModal === "add" ? t("add") : t("edit")} ${t("WATER DRY")}`}
      visible={visible}
      width="630px"
      okButtonProps={{
        form: "form-water-dry-management",
        key: "submit",
        htmlType: "submit",
        type: "primary",
      }}
      ...
      forceRender
    >
      <Form id="form-water-dry-management" form={form} onFinish={handleSubmit}>
        <div>
          <span>{t("waterDryId")}</span>
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.waterDryLao,
                  t
                );
              },
            },
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.waterDryEng,
                  t
                );
              },
            },
