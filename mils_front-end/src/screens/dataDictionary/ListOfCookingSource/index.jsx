...
let timeOut = "";
export default function ListOfCookingSource(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_COOKING_SOURCE);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.LIST_OF_COOKING_SOURCE) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  const columns = [
    {
      title: t("cookingSourceId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("cookingSourceLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_COOKING_SOURCE}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_COOKING_SOURCE,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFCOOKINGSOURCE")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_COOKING_SOURCE}?keyword=${keyword}`);
        }}
      />
    </div>
  );
