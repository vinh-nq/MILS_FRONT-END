...
let timeOut = "";
export default function ListOfLevel(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_LEVEL);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.LIST_OF_LEVEL) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_LEVEL}?${value}page=${values}`);
    setPage(values);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFLEVEL")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_LEVEL}?keyword=${keyword}`);
        }}
      />
    </div>
  );
