...
let timeOut = "";
export default function ListOfSchoolEnroll(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_SCHOOL_ENROLL);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (
        `${location.pathname}${location.search}` === PATH.LIST_OF_SCHOOL_ENROLL
      ) {
        setKeyword("");
      }
    });
  ...
  const columns = [
    {
      title: t("schoolEnrollId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("schoolEnrollLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_SCHOOL_ENROLL}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_SCHOOL_ENROLL,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFSCHOOLENROLL")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_SCHOOL_ENROLL}?keyword=${keyword}`);
        }}
      />
    </div>
  );
