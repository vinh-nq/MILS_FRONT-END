...
let timeOut = "";
export default function ListOfSchoolType(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_SCHOOL_TYPE);
    } else {
      setPage(pageCheck);
    }
  ...
  const columns = [
    {
      title: t("schooldTypeId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("schooldTypeLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_SCHOOL_TYPE}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_SCHOOL_TYPE,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFSCHOOLTYPE")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_SCHOOL_TYPE}?keyword=${keyword}`);
        }}
      />
    </div>
  );
