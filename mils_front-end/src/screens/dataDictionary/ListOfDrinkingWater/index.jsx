...
let timeOut = "";
export default function ListOfDrinkingWater(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_DRINKING_WATER);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (
        `${location.pathname}${location.search}` === PATH.LIST_OF_DRINKING_WATER
      ) {
        setKeyword("");
      }
    });
  ...
  const columns = [
    {
      title: t("drinkingWaterId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("drinkingWaterLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_DRINKING_WATER}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_DRINKING_WATER,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFDRINKINGWATER")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_DRINKING_WATER}?keyword=${keyword}`);
        }}
      />
    </div>
  );
