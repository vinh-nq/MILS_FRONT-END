...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertRelation(
        value,
        `Insert New Relation [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  const handleEditItem = async (value) => {
    await dataDictionaryApi
      .UpdateRelation(
        value,
        `Update Relation[${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      title={`${typeModal === "add" ? t("add") : t("edit")} ${t("RELATION")}`}
      visible={visible}
      width="630px"
      okButtonProps={{
        form: "form-relation-management",
        key: "submit",
        htmlType: "submit",
        type: "primary",
      }}
      ...
      forceRender
    >
      <Form id="form-relation-management" form={form} onFinish={handleSubmit}>
        <div>
          <span>{t("relationId")}</span>
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.relationLao,
                  t
                );
              },
            },
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.relationEng,
                  t
                );
              },
            },
