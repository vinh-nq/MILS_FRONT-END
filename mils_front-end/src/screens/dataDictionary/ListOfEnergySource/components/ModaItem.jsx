...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertEnergySource(
        value,
        `Insert New Energy Source [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  const handleEditItem = async (value) => {
    await dataDictionaryApi
      .UpdateEnergySource(
        value,
        `Update Energy Source [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      ...
      width="630px"
      okButtonProps={{
        form: "form-energysource-management",
        key: "submit",
        htmlType: "submit",
        type: "primary",
      }}
    ...
    >
      <Form
        id="form-energysource-management"
        form={form}
        onFinish={handleSubmit}
      >
        <div>
          <span>{t("energySourceId")}</span>
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  ...
                  value,
                  {
                    ...objectValidateForm.energySourceId,
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.energySourceLao,
                  t
                );
              },
            },
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.energySourceEng,
                  t
                );
              },
            },
