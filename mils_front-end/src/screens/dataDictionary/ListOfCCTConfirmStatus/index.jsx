...
let timeOut = "";
export default function ListOfCCTConfirmStatus(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.CCT_CONFIRM_STATUS);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.CCT_CONFIRM_STATUS) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  const columns = [
    {
      title: t("CCTConfirmStatusId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("CCTConfirmStatusLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.CCT_CONFIRM_STATUS}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.CCT_CONFIRM_STATUS,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("CCTCONFIRMSTATUS")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.CCT_CONFIRM_STATUS}?keyword=${keyword}`);
        }}
      />
    </div>
  );
