...
function ModaItem(props) {
  ...
  return (
    <Modal
      title={`${typeModal === "add" ? t("add") : t("edit")} ${t("LANDTYPE")}`}
      visible={visible}
      width="630px"
      okButtonProps={{
      ...
      forceRender
    >
      <Form id="form-management" form={form} onFinish={handleSubmit}>
        <div>
          <span>{t("landTypeId")}</span>
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.landTypeLao,
                  t
                );
              },
            },
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.landTypeEng,
                  t
                );
              },
            },
