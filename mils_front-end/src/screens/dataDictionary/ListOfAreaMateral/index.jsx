...
let timeOut = "";
export default function ListOfAreaMateral(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_AREA_MATERAL);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.LIST_OF_AREA_MATERAL) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  const columns = [
    {
      title: t("areaMateralId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("areaMateralLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_AREA_MATERAL}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_AREA_MATERAL,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFAREAMATERAL")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_AREA_MATERAL}?keyword=${keyword}`);
        }}
      />
    </div>
  );
