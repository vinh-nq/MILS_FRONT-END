...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertPlotStatus(
        value,
        `Insert New Plot Status [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  const handleEditItem = async (value) => {
    await dataDictionaryApi
      .UpdatePlotStatus(
        value,
        `Update Plot Status [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      title={`${typeModal === "add" ? t("add") : t("edit")} ${t("PLOTSTATUS")}`}
      visible={visible}
      width="630px"
      okButtonProps={{
        form: "form-plotStatus-management",
        key: "submit",
        htmlType: "submit",
        type: "primary",
      }}
      ...
      forceRender
    >
      <Form id="form-plotStatus-management" form={form} onFinish={handleSubmit}>
        <div>
          <span>{t("plotStatusId")}</span>
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.plotStatusLao,
                  t
                );
              },
            },
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.plotStatusEng,
                  t
                );
              },
            },
