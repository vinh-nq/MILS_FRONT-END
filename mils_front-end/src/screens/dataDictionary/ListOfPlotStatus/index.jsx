...
let timeOut = "";
export default function ListOfPlotStatus(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_PLOT_STATUS);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.LIST_OF_PLOT_STATUS) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  useEffect(() => {
    ...
    fetchDataPlotStatus(props.location);
  }, [props.location]);

  const fetchDataPlotStatus = async (location) => {
  ...
  const columns = [
    {
      title: t("plotStatusId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("plotStatusLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_PLOT_STATUS}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_PLOT_STATUS,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFPLOTSTATUS")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_PLOT_STATUS}?keyword=${keyword}`);
        }}
      />
    </div>
  );
