...
let timeOut = "";
export default function ListOfProvince(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_PROVINCE);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.LIST_OF_PROVINCE) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_PROVINCE}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_PROVINCE,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LIST OF PROVINCE")}</span>
      ...
      <ModaItem
        ...
        typeModal={typeModal}
        arrayDuplicate={listProvince.map((el) => `${el.Id}`)}
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_PROVINCE}?keyword=${keyword}`);
        }}
      />
    </div>
  );
