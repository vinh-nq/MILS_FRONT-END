...
export default function ListOfPlotCause(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_PLOT_CAUSE);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.LIST_OF_PLOT_CAUSE) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  useEffect(() => {
    ...
    fetchDataPlotCause(props.location);
  }, [props.location]);

  const fetchDataPlotCause = async (location) => {
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_PLOT_CAUSE}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_PLOT_CAUSE,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_PLOT_CAUSE}?keyword=${keyword}`);
        }}
      />
    </div>
  );
