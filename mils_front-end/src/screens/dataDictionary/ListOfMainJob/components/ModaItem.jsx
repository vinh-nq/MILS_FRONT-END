...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertMainJob(
        value,
        `Insert New Main Job [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  const handleEditItem = async (value) => {
    await dataDictionaryApi
      .UpdateMainJob(
        value,
        `Update Main Job [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      title={`${typeModal === "add" ? t("add") : t("edit")} ${t("MAINJOB")}`}
      visible={visible}
      width="630px"
      okButtonProps={{
