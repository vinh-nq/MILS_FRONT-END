...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertMainGoodsServices(
        value,
        `Insert New Main Good Services [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  const handleEditItem = async (value) => {
    await dataDictionaryApi
      .UpdateMainGoodsServices(
        value,
        `Update Main Good Services [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      ...
      width="630px"
      okButtonProps={{
        form: "form-mainGoodAndService-management",
        key: "submit",
        htmlType: "submit",
        type: "primary",
      }}
    ...
    >
      <Form
        id="form-mainGoodAndService-management"
        form={form}
        onFinish={handleSubmit}
      >
        <div>
          <span>{t("mainGoodAndServiceId")}</span>
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  ...
                  value,
                  {
                    ...objectValidateForm.mainGoodAndServiceId,
        ...
        <div>
          <span>{t("mainGoodAndServiceLao")}</span>
          <span style={{ paddingLeft: "3px", color: "red" }}>*</span>
        </div>
        <Form.Item
          name="ValueOfLao"
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.mainGoodAndServiceLao,
                  t
                );
              },
            },
        ...
        <div>
          <span>{t("mainGoodAndServiceEng")}</span>
          <span style={{ paddingLeft: "3px", color: "red" }}>*</span>
        </div>
        <Form.Item
          name="ValueOfEng"
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.mainGoodAndServiceEng,
                  t
                );
              },
            },
