...
let timeOut = "";
export default function ListOfBorrowReason(props) {
  ...
  useEffect(() => {
    ...
    if (!regexTemplate.NUMBER.test(pageCheck)) {
      props.history.push(PATH.LIST_OF_BORROW_REASON);
    } else {
      setPage(pageCheck);
    }
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (
        `${location.pathname}${location.search}` === PATH.LIST_OF_BORROW_REASON
      ) {
        setKeyword("");
      }
    });
  ...
  const columns = [
    {
      title: t("borrowReasonId"),
      dataIndex: "Id",
      key: "Id",
    },
    {
      title: t("borrowReasonLao"),
  ...
  const onChangePage = (values) => {
    ...
    props.history.push(`${PATH.LIST_OF_BORROW_REASON}?${value}page=${values}`);
    setPage(values);
  };
  ...
  const onSearchChange = (e) => {
    ...
    timeOut = setTimeout(() => {
      ...
      props.history.push({
        pathName: PATH.LIST_OF_BORROW_REASON,
        search: link,
      });
    }, 400);
  };
  ...
  return (
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between">
        <span className="h5 mb-0">{t("LISTOFBORROWREASON")}</span>
      ...
      <ModaItem
        ...
        reloadData={(keyword) => {
          history.push(`${PATH.LIST_OF_BORROW_REASON}?keyword=${keyword}`);
        }}
      />
    </div>
  );
