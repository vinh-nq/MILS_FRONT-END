...
function ModaItem(props) {
  ...
  const handleAddNew = async (value) => {
    await dataDictionaryApi
      .InsertBorrowReason(
        value,
        `Insert New Borrow Reason [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  const handleEditItem = async (value) => {
    await dataDictionaryApi
      .UpdateBorrowReason(
        value,
        `Update Borrow Reason [${value.Id}] - ${value.ValueOfEng}`
      )
      .then((res) => {
        handleCanncel();
        message.success({
  ...
  return (
    <Modal
      ...
      width="630px"
      okButtonProps={{
        form: "form-borrowReason-management",
        key: "submit",
        htmlType: "submit",
        type: "primary",
      }}
    ...
    >
      <Form
        id="form-borrowReason-management"
        form={form}
        onFinish={handleSubmit}
      >
        <div>
          <span>{t("borrowReasonId")}</span>
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  ...
                  value,
                  {
                    ...objectValidateForm.borrowReasonId,
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.borrowReasonLao,
                  t
                );
              },
            },
          ...
          rules={[
            {
              validator(rule, value) {
                return handleValidateFrom(
                  rule,
                  value,
                  objectValidateForm.borrowReasonEng,
                  t
                );
              },
            },
