...
export default function ListDistrictApprove(props) {
  ...
  const listItems = useSelector(
    (state) => state.districtApproveReducer.listItems
  );
  const dataLanguage =
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (`${location.pathname}${location.search}` === PATH.DISTRICT_APPROVE) {
        setKeyword("");
      }
    });
  }, [history]);
  ...
  useEffect(() => {
    ...
    const fetchDataListHouseHold = () => {
      return approveHouseHoldApi.GetHHForDistrict({
          ...
          <div className="d-flex justify-content-end" style={{ minWidth: 80 }}>
            <Button
              ...
              onClick={() => {
                ...
                history.push(
                  `${PATH.HOUSEHOLD_DETAIL_ON_DISTRICT_APPROVE}?hh_code=${record.hh_code}`
                );
              }}
            />
          </div>
    ...
    onSelect: (record, selected) => {
      if (listItems.length > 0) {
        if (
          ...
          if (selected) {
            dispatch({
              type: actionRedux.ADD_DISTRICT_APPROVE,
              payload: record,
            });
          } else {
            dispatch({
              type: actionRedux.DELETE_DISTRICT_APPROVE,
              payload: record,
            });
          }
        } else {
  ...
  const onChangePage = (number) => {
    ...
    history.push(
      `${PATH.DISTRICT_APPROVE}?page=${number}${region}${keywordData}`
    );
  };
  ...
  const onSearchHandle = () => {
    ...
    const keywordData = keyword ? `&keyword=${keyword}` : "";
    history.push(`${PATH.DISTRICT_APPROVE}?page=1${region}${keywordData}`);
  };
    ...
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between row">
        <div className="col-xl-4 col-md-5 col-12">
          <span className="h5 mb-0">{t("DISTRICTAPPROVE")}</span>
        </div>
      </div>
      <Divider />
      <div className="row">
          ...
          {listItems.length === 0 ? null : (
            <>
              <Button
                onClick={() => {
                  dispatch({
                    type: actionRedux.SET_LIST_DISTRICT_APPROVE,
                    payload: [],
                  });
                }}
                className="mr-2"
                        ...
                        const sendRequest = async () => {
                          ...
                          return await approveHouseHoldApi
                            .DistrictReject(
                              ...
                              `District Reject : ${listItems.length} Households`
                            )
                            .then((res) => {
                              message.success({
                      ...
                      const sendRequest = async () => {
                        ...
                        return await approveHouseHoldApi
                          .DistrictApprove(
                            ...
                            `District Approve: ${listItems.length} Households`
                          )
                          .then((res) => {
                            message.success({
