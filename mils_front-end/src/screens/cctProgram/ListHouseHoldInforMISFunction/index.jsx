...
export default function ListHouseHoldInforMISFunction(props) {
  ...
  const { confirm } = Modal;
  const listItems = useSelector((state) => state.CMRequestReducer.listItems);
  const dataLanguage =
  ...
  useEffect(() => {
    ...
    const fetchDataListHouseHold = () => {
      return approveHouseHoldApi.GetHHToSendRequest({
          ...
          <div className="d-flex justify-content-end" style={{ minWidth: 80 }}>
            <Button
              ...
              onClick={() => {
                ...
                history.push(
                  `${PATH.HOUSEHOLD_DETAIL_ON_CREATE_LIST_HOUSEHOLD_REQUEST}?hh_code=${record.hh_code}`
                );
              }}
            />
          </div>
  ...
  const rowSelection = {
    ...
    getCheckboxProps: (item) => ({
      disabled: item.StatusApproveDistrict === 1,
    }),
    onSelect: (record, selected) => {
      if (selected) {
        dispatch({
          type: actionRedux.ADD_CM_REQUEST_ITEM,
          payload: record,
        });
      } else {
        dispatch({
          type: actionRedux.DELETE_CM_REQUEST_ITEM,
          payload: record,
        });
      }
    },
    onSelectAll: (selected, selectedRows, changeRows) => {
      if (selected) {
        dispatch({
          type: actionRedux.SET_LIST_CM_REQUEST,
  ...
  const onChangePage = (number) => {
    ...
    history.push(
      `${PATH.CREATE_LIST_HOUSEHOLD_REQUEST}?page=${number}${region}${keywordData}`
    );
  };
  ...
  const onSearchHandle = () => {
    ...
    const keywordData = keyword ? `&keyword=${keyword}` : "";
    history.push(
      `${PATH.CREATE_LIST_HOUSEHOLD_REQUEST}?page=1${region}${keywordData}`
    );
  };
    ...
    <div className="w-100 h-100">
      ...
      <div className="d-flex flex-row align-items-center justify-content-between row">
        <div className="col-xl-4 col-md-5 col-12">
          <span className="h5 mb-0">{t("CREATELISTHOUSEHOLDREQUEST")}</span>
        </div>
      </div>
      <Divider />
      <div className="row">
